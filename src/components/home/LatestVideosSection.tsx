            ...
            <div className="container mx-auto px-4 lg:px-8 relative z-10">
                ...
                <FadeInStagger className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                    {videos.map((video: YouTubeVideo) => (
                        <FadeIn key={video.id.videoId}>
                            <Link
                                href={`https://www.youtube.com/watch?v=${video.id.videoId}`}
                                target="_blank"
                                className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-red-900/5 transition-all duration-300 transform hover:-translate-y-1 h-full"
                            >
                                {/* Thumbnail Container */}
